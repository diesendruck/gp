...
% MO MODIFIED: for (2, n)-dimensional data.
% h = fminsearch(@(h) Resid_Sq_Gauss_Ker_Reg(h, x(:, inx1), y(:, inx1), ...
%     x(:, inx2), y(:, inx2)), h0);

% Original active function:
h = fmincon(@(h) Resid_Sq_Gauss_Ker_Reg(h, x(:, inx1), y(:, inx1), ...
    x(:, inx2), y(:, inx2)), h0, [], [], [], [], d/20, 2*d);
