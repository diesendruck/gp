...
%% RUN EXPERIMENT ONCE FOR A GIVEN SHAPE AND SAMPLE.
for i = 1:num_samples
    single_run_start = tic;
    ...
    if do_plot
        ...
        mesh(xt1, xt2, yq_conv); hold on;
        plot3(x_nsy(:, 1), x_nsy(:, 2), y_nsy, 'r.', 'MarkerSize', 10);
        title(sprintf('AvgGP Proj (RMSE = %s)', num2str(rmse_gp_conv, '%0.3f')));
        % Standardize z-axis.
        zlim(zl);
    end

    gp_conv_time_elapsed = toc(gp_conv_start_time);


    %% COMPUTE SEN ESTIMATE AND ITS MSE.
    
    y_sen_test = project_to_convex(length(tt), d, tt, y_nsy, eps1, eps2);
    rmse_sen = sqrt(1/length(tt) * norm(y_sen_test(:) - ytruth_on_test)^2);

    if do_plot
        % Plot avg MCMC over original data.
        subplot(3, 3, 7);
        plot3(x_nsy(:, 1), x_nsy(:, 2), y_nsy, 'r.', ...
            'MarkerSize', 10); hold on;
        plot3(x_nsy(:, 1), x_nsy(:, 2), y_sen_test, 'b.', ...
            'MarkerSize', 20);
        grid on;
    ...
    % GETTING ERRORS LIKE THIS, WITHOUT JITTER:
    ...
    single_run_time_elapsed = toc(single_run_start);
    
    
    %% SEND RESULTS.
    sendmail('momod@utexas.edu', ...
        sprintf('UPDATE %s: RMSE Comparisons', platform), ...
        sprintf(strcat('Global iter: %s\n', ...
                       ...
                       'mbcr_time:              %s\n', ...
                       'run_time:              %s\n'), ...
            num2str(i), shape, num2str(rmse_gp, '%0.2f'), ...
            num2str(rmse_gp_conv, '%0.2f'), num2str(rmse_kern, '%0.2f'), ...
            num2str(rmse_kern_conv, '%0.2f'), num2str(rmse_sen, '%0.2f'),...
            num2str(rmse_cap, '%0.2f'), num2str(rmse_mbcr, '%0.2f'), ...
            num2str(gp_conv_time_elapsed, '%0.2f'), ...
            num2str(mbcr_time_elapsed, '%0.2f'), ...
            num2str(single_run_time_elapsed, '%0.2f')));
end
